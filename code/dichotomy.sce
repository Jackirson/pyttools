...
function [ answer ] = ldichotomy( p, p_level, l, K )
...
// ===================
function  [ P_loser ] = lfindPLoser(p_init, qLoserIndex, qSelSize)
    ...
    while ( p_step > eps_p )
        p_old = p_i;
        if ldichotomy(p_init, p_i(1)+p_step, qLoserIndex, qSelSize)
            // +1/2 of current step
            p_i(1) = p_i(1) + p_step;
            p_step = 0.5 * p_step;           
        else
